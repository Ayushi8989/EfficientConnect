...
function createOffer(peerUsername, receiver_channel_name){
    ...
    peer.addEventListener('icecandidate', (event) => {
        ...
        // Get local video frame
        var canvas = document.createElement('canvas');
        var ctx = canvas.getContext('2d');
        canvas.width = localVideo.videoWidth;
        canvas.height = localVideo.videoHeight;
        ctx.drawImage(localVideo, 0, 0, canvas.width, canvas.height);
        var localFrame = canvas.toDataURL('image/jpeg');

        // Apply adaptive sampling and compression
        var compressedFrame = adaptiveSamplingAndCompression(localFrame);

        sendSignal('new-offer', {
            'sdp' : peer.localDescription,
            'receiver_channel_name' : receiver_channel_name,
            'frame': compressedFrame  // Send the compressed frame to the remote peer
        })
    })
...
function createAnswer(offer, peerUsername, receiver_channel_name){
    ...
    peer.addEventListener('icecandidate', (event) => {
        ...
        adaptiveSamplingAndCompression(localFrame, function(compressedFrame) {
...
function adaptiveSamplingAndCompression(frameDataUrl, callback) {
    var img = new Image();
    ...
    img.onload = function() {
        ...
        var originalSize = frameDataUrl.length;
